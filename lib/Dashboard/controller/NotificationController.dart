...
class NotificationController extends GetxController {
  ...
  getNotificationListLoadingNetworkApi(int start) async {
    var response = await BaseClient()
        .get(getNotificationList +
            "?lng=eng&limit=${15}&page=${start}&user_id==${_storage.read(AppConstant.id)}")
        .catchError(BaseController().handleError);

    if (jsonDecode(response)["status"] == 1) {
      if (isLoadingNotificationPage.value == true) {
        notificationModel.value.data!
            .addAll(notificationModelFromJson(response).data!);
  ...
  getNotificationListNetworkApi() async {
    ...
    var response = await BaseClient()
        .get(getNotificationList +
            "?lng=eng&limit=${15}&page=0&user_id=${_storage.read(AppConstant.id)}")
        .catchError(BaseController().handleError);
    Get.context!.loaderOverlay.hide();
    print("ksjdfhjh" +
        getNotificationList +
        "?lng=eng&limit=${10}&page=0&user_id==${_storage.read(AppConstant.id)}");
    if (jsonDecode(response)["status"] == 1) {
