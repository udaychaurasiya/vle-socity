...
import 'package:vlesociety/Dashboard/view/quizeQA/SuperQuestion.dart';
...
class QuizeController extends GetxController
{
 ...
 QuizTestNetworkApi()async{
    ...
    if(jsonDecode(response)["status"]==1)
    {
      Get.off(()=> Supershowresult(selectedAnsList,quize_id: jsonDecode(response)["Data"]["tbl_quiz_id"].toString(),
        ttCorrect: jsonDecode(response)["Data"]["ttl_correct"].toString(),
        ttInCorrect:jsonDecode(response)["Data"]["ttl_wrong"].toString(),));
      return;
    }
    BaseController().errorSnack(jsonDecode(response)["message"]);
  }
