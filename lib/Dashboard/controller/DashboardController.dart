...
import 'package:vlesociety/CSC/Model/SiteLinkModel.dart';
...
import 'package:vlesociety/Dashboard/model/Quiz_Model/LiveChatModel.dart';
import 'package:vlesociety/Dashboard/model/SpecellyNotificationModel.dart';
...
import '../view/Earning/Model/CouponModel.dart';
import '../view/Earning/Refer/ReferAndEarn.dart';
...
class DashboardController extends GetxController {
  ...
  TextEditingController etAmount2=TextEditingController();
  ...
  var livechat = LiveChatModel().obs;
  var coupopCode = CouponCodeModel().obs;
  var siteLink = SiteLinkModel().obs;
  var snotifiaction = SpecallyNotificationModel().obs;
  ...
  Future<bool> postCouponCodeApi() async {
    var bodyRequest =
    {
      "lng": language,
      "user_id": _storage.read(AppConstant.id),
      "amount": "${int.parse(etAmount2.text)*ammount2}",
      "remark": Remark.text.toString().trim(),
      "coupon_amount":etAmount2.text.toString()
    };
    Get.context!.loaderOverlay.show();
    var response = await BaseClient()
        .post(postGenerateCoupon, bodyRequest)
        .catchError(BaseController().handleError);
    print("${bodyRequest}dbfuygyfg");
    Get.context!.loaderOverlay.hide();
 ...
 getCouponCodeNetworkApi() async
  {
    Get.context!.loaderOverlay.show();
    var response = await BaseClient()
        .get(getCouponCode+"?lng=eng&user_id=${_storage.read(AppConstant.id)}&limit=500&page=0")
        .catchError(BaseController().handleError);
    Get.context!.loaderOverlay.hide();
    print(response+"elkfhigf");
    if (jsonDecode(response)["status"] == 1) {
      coupopCode.value = couponCodeModelFromJson(response);
      return;
    }
    coupopCode.value = couponCodeModelFromJson(response);
    BaseController().errorSnack(jsonDecode(response)["message"]);
  ...
  Future<bool>deleteCommentNetworkApi(String id) async
  {
    Get.context!.loaderOverlay.show();
    var response = await BaseClient()
        .delete(deleteCommentApi + "?lng=eng&user_id=${_storage.read(AppConstant.id)}&community_answer_id=${id}")
        .catchError(BaseController().handleError);
    Get.context!.loaderOverlay.hide();
    if (jsonDecode(response)["status"] == 1) {
        BaseController().successSnack(jsonDecode(response)["message"].toString());
  ...
  Future<bool>deletePostCommunityApi(String community_id) async
  {
    print("resopnse123==>>"+community_id);
    Get.context!.loaderOverlay.show();
    var response = await BaseClient()
        .delete(deletePostApi + "?lng=eng&user_id=${_storage.read(AppConstant.id)}&community_id=${community_id}")
        .catchError(BaseController().handleError);
    print("resopnse12==>>"+response);
    Get.context!.loaderOverlay.hide();
    if (jsonDecode(response)["status"] == 1) {
      BaseController().successSnack(jsonDecode(response)["message"].toString());
      getCommunityNetworkApi();
  ...
  getLiveChatNetworkApi() async {
    var response = await BaseClient()
        .get(liveChat + "?lng=eng&user_id=${_storage.read(AppConstant.id)}")
        .catchError(BaseController().handleError);

    if (jsonDecode(response)["status"] == 1) {
      livechat.value = liveChatModelFromJson(response);
      return;
    }
    livechat.value = liveChatModelFromJson(response);
    BaseController().errorSnack(jsonDecode(response)["message"]);
  ...
  getSiteLinkNetworkApi() async {
    var response = await BaseClient()
        .get(getSiteLinkApi + "?lng=eng")
        .catchError(BaseController().handleError);
         print(response+"efjhuiugf");
    if (jsonDecode(response)["status"] == 1) {
      siteLink.value = siteLinkModelFromJson(response);
      return;
    }
    // BaseController().errorSnack(jsonDecode(response)["message"]);
  }

  getSpeccallyNetworkApi() async {
    var response = await BaseClient()
        .get(getSpeciellyNotification + "?lng=eng")
        .catchError(BaseController().handleError);
         print(response+"efuhyug");
    if (jsonDecode(response)["status"] == 1) {
      snotifiaction.value = specallyNotificationModelFromJson(response);
      return;
    }
  }
  ...
  Future<bool> postRiplyReportNetworkApi(
      String post_report_id, String community_answer) async {
    var bodyRequest = {
      "lng": language,
      "user_id": _storage.read(AppConstant.id),
      "post_community_answer": community_answer,
      "post_report_id": post_report_id
    };
    ...
    Get.context!.loaderOverlay.show();
    var response = await BaseClient()
        .post(postAnswerReport, bodyRequest)
        .catchError(BaseController().handleError);
    Get.context!.loaderOverlay.hide();
    print("dfbijbdkbd");
    print(response);
...
//
  ...
  Future<bool> postRedeemNetworkApi(String rdeemType) async {
    var bodyRequest =
    {
      "lng": language,
      "user_id": _storage.read(AppConstant.id),
      "amount": "${int.parse(etAmount.text)*ammount}",
