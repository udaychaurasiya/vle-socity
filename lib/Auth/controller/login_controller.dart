...
print(fcmToken.toString()+"dihuuigf");
  print("vsdfbfd"+etMobile.text+"    "+response);
  print(response);
  if(jsonDecode(response)["status"]==1)
    {
      BaseController().successSnack2(jsonDecode(response)["message"]+" "+" ");
      Get.to(()=>OtypVerifyPage(id: jsonDecode(response)["Data"]["ID"],));
          return;
    }
  ...
  getStateNetworkApi()async{
    ...
    if(jsonDecode(response)["status"]==1)
    {
      ...
      int ind =stateData.value.data!.indexWhere((element) => element.stateId == GetStorage().read(AppConstant.stateId).toString());
      if (ind != -1)
      {
        selectedState = stateData.value.data![ind];
        etSate.text =stateData.value.data![ind].stateId!;
        getCityNetworkApi(
            stateData
  ...
  getCityNetworkApi(String id)async{
    ...
    if(jsonDecode(response)["status"]==1)
    {
     ...
     if (cityModel.value.data !=
         null) {
       int ind =cityModel.value.data!
           .indexWhere((element) =>
       element.id ==
           GetStorage()
               .read(AppConstant
  ...
  setEtDataController()
  {
    ...
    etZip.text=_storage.read(AppConstant.zip)??"";
    etblock.text=_storage.read(AppConstant.block)??"";
